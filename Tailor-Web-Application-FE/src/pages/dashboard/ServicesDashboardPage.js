...
// sections
import { ServiceListHead, ServiceListToolbar  } from '../../sections/@dashboard/services';
// data
import {
  setServiceError,
  setServiceStatus,
...
const TABLE_HEAD = [
  ...
  { id: 'estDuration', label: 'EstDuration', alignRight: false },
  { id: 'isAvailable', label: 'IsAvailable', alignRight: false },
...
export default function ServiceDashboardPage() {
  ...
  useEffect(() => {
    if (services && services.length > 0) {
      const updatedFilteredServices = applySortFilter(services, getComparator(order, orderBy), filterName);
      setFilteredServices(updatedFilteredServices);
    }
  }, [services, order, orderBy, filterName]);

  const isNotFound = !filteredServices.length && !!filterName;
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                />
                <TableBody>
                  {filteredServices.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, name, description, price, estDuration, isAvailable, images, discount } = row;
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">{estDuration}</TableCell>
...
>
      ...
      {showModal && <ServiceModal open={showModal} onClose={handleOnClose}/>}
      ...
      {showConfirmation && (
        <BasicModal
          ...
          title={<FormattedMessage id = {"lbl.delete-confirmation"}/>}
